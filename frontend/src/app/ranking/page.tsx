...
export default function RankingPage() {
  ...
  const S:Record<string,React.CSSProperties> = {
    ...
    wrap:{paddingTop:100,paddingBottom:80,padding:"100px 40px 80px",maxWidth:900,margin:"0 auto",position:"relative",zIndex:1},
    header:{textAlign:"center",marginBottom:40},
    title:{fontFamily:"var(--font-display)",fontSize:"clamp(2.5rem,6vw,4rem)",fontWeight:300,letterSpacing:"-0.01em",marginBottom:8},
    livePill:{display:"inline-flex",alignItems:"center",gap:7,fontSize:"0.72rem",color:live?"#4CAF50":"var(--text-muted)",letterSpacing:"0.1em"},
    tabs:{display:"flex",gap:8,justifyContent:"center",marginBottom:36},
    podium:{display:"flex",alignItems:"flex-end",justifyContent:"center",gap:12,marginBottom:40,height:160},
    podiumItem:{
      flex:1,maxWidth:140,
      background:"var(--glass)",border:"1px solid var(--border)",
      borderRadius:"16px 16px 0 0",
      display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end",
      paddingBottom:12,position:"relative",overflow:"hidden",backdropFilter:"blur(10px)",
    },
    list:{display:"flex",flexDirection:"column",gap:10},
    row:{
      display:"grid",gridTemplateColumns:"48px 48px 1fr auto auto",alignItems:"center",gap:14,
      padding:"14px 20px",
      background:"var(--glass)",border:"1px solid var(--border)",
    ...
    votesVal:{fontFamily:"var(--font-display)",fontSize:"1.1rem",fontWeight:600,color:"var(--gold-light)"},
    votesLabel:{fontSize:"0.65rem",color:"var(--text-muted)"},
    voteBtn:{
